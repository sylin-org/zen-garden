...
//!
//! Translates canonical OpenAI-shaped types to/from the Anthropic Messages
//! API. Reuses the proven translation logic from `anthropic.rs` (the Offering
//! adapter) and wraps it in the `InferenceAdapter` trait.
//!
//! Key translations (verified against Anthropic docs):
//! - Auth: `x-api-key` header + `anthropic-version: 2023-06-01`
//! - System prompt: extracted from messages → top-level `system` field
//! - `max_tokens`: required by Anthropic, defaults to 4096
//! - Temperature: clamped to 0.0-1.0 (Anthropic range)
//! - `stop` → `stop_sequences` (array)
//! - Message alternation: strict user/assistant, merge consecutive same-role
//! - Tool definitions: unwrap OpenAI wrapper → Anthropic `{name, description, input_schema}`
//! - Tool args in response: `input` (object) → `function.arguments` (JSON string)
//! - Stop reasons: `end_turn` → `stop`, `max_tokens` → `length`, `tool_use` → `tool_calls`
//! - Usage: `input_tokens` → `prompt_tokens`, `output_tokens` → `completion_tokens`
//! - Stream: Anthropic named events (`event: type\ndata: json`) → `InferenceChunk`
...
/// Anthropic inference adapter — stateless protocol translator.
pub struct AnthropicInferenceAdapter {
    http: Client,
}

impl AnthropicInferenceAdapter {
    pub fn new() -> Self {
...
impl InferenceAdapter for AnthropicInferenceAdapter {
    fn infer(
        &self,
        ctx: &AdapterContext,
    ...
    fn infer_stream(
        ...
        Box::pin(async move {
            ...
            Ok(Box::pin(AnthropicSseStream::new(stream, model))
...
fn build_anthropic_request(model: &str, req: &InferenceRequest, stream: bool) -> Value {
    // Start with the canonical request as a JSON value to manipulate.
...
/// ```
///
/// The `event:` line precedes the `data:` line. Segments are separated by `\n\n`.
    ...
    fn poll_next(
        ...
        loop {
            ...
            if let Some(pos) = find_double_newline(&this.buffer) {
                let segment_bytes = this.buffer.split_to(pos + 2); // include the \n\n
                ...
                match event_type.as_str() {
                    "message_start" => {
                        // Initial message shell with usage. Emit a chunk with role only.
                    ...
                    "content_block_start" => {
                        // Track new content block — could be text or tool_use.
                        ...
                        // No chunk to emit — continue reading.
                        continue;
                    }
                    ...
                    "content_block_stop" => {
                        // If this was a tool_use block, emit the completed tool call.
                        let idx = data
                            .get("index")
                            .and_then(|i| i.as_u64())
                            .unwrap_or(0) as usize;
...
mod tests {
    ...
    fn request_clamps_temperature_and_maps_stop() {
        ...
        assert!((temp - 1.0).abs() < f64::EPSILON, "temperature should be clamped to 1.0");
