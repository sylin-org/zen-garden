...
try {
    ...
    Write-Host "OK Moss is running: $health" -ForegroundColor Green
} catch {
    Write-Host "X Moss is not running at $MossEndpoint" -ForegroundColor Red
...
try {
    ...
    Write-Host "OK Services endpoint works" -ForegroundColor Green
...
try {
    ...
    Write-Host "OK Quiet mode works" -ForegroundColor Green
    if ($response.suggestions -and $response.suggestions.Count -gt 0) {
        Write-Host "   !  Suggestions not suppressed (count: $($response.suggestions.Count))" -ForegroundColor Yellow
    } else {
        Write-Host "   Suggestions suppressed: OK" -ForegroundColor Gray
    }
} catch {
    Write-Host "X Failed: $_" -ForegroundColor Red
    exit 1
...
try {
    ...
    Write-Host "OK Stone endpoint works" -ForegroundColor Green
...
try {
    ...
    Write-Host "OK Garden endpoint works" -ForegroundColor Green
...
try {
    ...
    Write-Host "OK Pond status endpoint works" -ForegroundColor Green
...
try {
    ...
    if ($LASTEXITCODE -eq 0 -or $output -match "No Zen Garden stones discovered") {
        Write-Host "OK Rake quiet flag works" -ForegroundColor Green
    } else {
        Write-Host "!  Rake execution had issues but flag accepted" -ForegroundColor Yellow
    }
} catch {
    Write-Host "X Failed: $_" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "OK Legacy endpoint still works" -ForegroundColor Green
