...
`koi-runtime` taken as `default-features = false`, documented in `docs/poc/guides/embedded.md`
...
- `docs/poc/CHANGELOG.md`: no entry for the koi 0.4 lean migration / dropped `docker`+`keyring` /
...
- **consumes koi via PATH DEPS** (local `../koi`, lean features) — a deliberate dogfooding choice
  *until koi stabilizes*, NOT crates. Clean-clone-from-crates is intentionally deferred; the
  switch-back procedure is in [`docs/poc/guides/koi-dependency.md`](../docs/poc/guides/koi-dependency.md).
...
## 2026-06-15 — prompt 03 (dead-code deletion sweep): skipped rows + doc refs + adjacent issues

Executed the 14-row kill list. **Skipped rows (proof command disagreed with the table — kill list was wrong, not the tree):**
- **Row 13 `cmd::TAKE_ROOT` / `MAKE` / `BROWSE`**: NOT stale — actively used by `install_service.rs`
  (TakeRoot), `make.rs`, `browse.rs`. Kept.
- **Row 13 ceremony/template "coming-soon manifest entries"**: NOT dead — `CeremonyCommand` /
  `TemplateCommand` are **routed live commands** (`route.rs:500-536`); their manifest entries +
  `cmd::CEREMONY`/`cmd::TEMPLATE` consts are in use. Kept.

**Kill-list corrections (deleted, but the table's proof was imprecise):**
- **Row 5 (stone.rs)**: the proof regex was over-broad (matched `StoneStatus`/`StoneResources`, which
  live in `types/`) AND missed the `domain/mod.rs` re-export site. Precise re-verify + green compile
  confirmed `Stone`/`Current`/`Environment`/`OsKind` genuinely dead (moss has its own `current` domain).
- **Rows 3-4**: `api_utils/sse.rs` was also deleted (it consumed the dying `events` module and had zero
  external consumers — moss/lantern use their own `event_bus`/`SseEvent`).

**Doc references to deleted items (for prompts 06/08 — not touched this session):**
- `json-transform`: `docs/archive/proposals/offering-sub-capabilities.md:227,495`,
  `docs/poc/proposals/unified-interface-language.md:1229`, `docs/notes/assessment-2026-06/shed-register.md:34`.
- `GardenApiResponse`/`GardenHttpClient`: `src/common/src/api_manifest.rs:110` (a `response_type` string,
  not a type use), `src/common/src/responses.rs:4` (comment).

**Adjacent pre-existing issues found (NOT caused by the sweep, NOT fixed — scope discipline):**
- **ollama orchestrator `cargo check --all-targets` fails** (lib+bin are green): `OllamaInstance` is
  missing field `compute_type` in test initializers — `placement.rs:155`, `recommendation.rs:521`,
  `reconciliation.rs:125`, `routing.rs:256`, `tiering.rs:64`. Pre-dates this sweep (no relation to the
  deleted `cluster`).
- Three pre-existing test-code warnings surfaced when common/moss recompiled (none caused by the sweep):
  `common/src/detection/inventory.rs:255` (`super::*` unused), `moss/src/domain/storage/bank_aggregate.rs:515`
  (`PinState` unused), `moss/src/domain/catalog/tests.rs:39` (`with_index` dead_code).

## 2026-06-15 — prompt 05 (security-baseline): cleanups done, mTLS build deferred

**Operator reshaped the design (gate):** no stone-local token; security model = "Pond present → use it
(mTLS); no Pond → open" (home-lab-open is acceptable/desirable); gate all mutating routes when Pond
active, exempt enrollment; deploy/upgrade stay open.

**Feasibility finding that changed the work:** rake/StoneApi talk plain HTTP `:7185` with no client cert
(`reqwest::Client::new()`), so "Pond secures writes" can't reach the CLI today. Realizing the operator's
model requires **teaching rake to speak Pond mTLS** to `:7183` (load the enrollment cert/key/ca from
`rake enrollment::certs_dir`, mutual-TLS). Operator chose to build that — but as a focused **fresh
session** (it's the largest security feature in the stash, and this session already did prompts 02/03/04
+ the koi migration). Plan: [`docs/notes/security-rake-mtls-plan.md`](security-rake-mtls-plan.md).

**Cleanups DONE this session (committed 7ef0307d, 6e871115, 67319fb0):**
- `changeme` default removed from rake pond invite/unlock/promote → require `--passphrase` (clear error).
- `banks/{moniker}/seeds` route drift fixed (mirrored into `configure()`).
- first-stone.md SSH "disabled by default" lie corrected (the preseed installs `ssh-server` via tasksel +
  NOPASSWD `stone` account → SSH is *enabled* by default with default creds).

**Accepted risks / follow-ups (for the mTLS session or later):**
- **deploy/upgrade stay open even when Pond active** (operator decision 3) — unauthenticated root
  code-push on `:7185` remains; accepted home-lab tradeoff, but the single biggest residual exposure.
- **Preseed SSH hardening** — disable password auth post-enrollment (out of scope here; doc-corrected only).
- **Interactive passphrase prompt** for invite/unlock/promote would beat `--passphrase` on the command
  line (avoids shell-history/process-args leakage) — optional UX nicety.

**Environment note:** the F: drive (931 GB NAS replica) hit **100% full** mid-session (`os error 112`),
blocking compiles; freed ~10 GB by removing zen's non-workspace build trees (orchestrator + native
`target-*` dirs). (Operator later reclaimed it — back to ~58%.)

**BLOCKER finding — moss does NO mTLS; delegate to koi.** During the build, discovered moss's :7183
listener is `ServerConfig::builder().with_no_client_auth()` (`bootstrap/tls.rs:101`, "mTLS deferred to
Phase 4") — moss never verifies client certs anywhere, and koi exposes no mTLS server builder as a
library (only the public `koi_certmesh::http::ClientCn` type). So "Pond secures writes via mTLS" rests on
unbuilt infrastructure: realizing it means building client-cert enforcement + peer-identity extraction.
Operator's call: **delegate that to koi** (correct — koi owns certmesh/PKI per the stack canon; zen must
not duplicate a rustls verifier). koi ALREADY has the exact code in its binary
(`koi/src/adapters/mtls.rs`: `WebPkiClientVerifier` from the certmesh CA + CN extraction + `ClientCn`
injection); it just needs promoting to a koi-certmesh **library** API. Koi prompt drafted:
[`koi-mtls-library-prompt.md`](koi-mtls-library-prompt.md). zen-side integration (moss delegates :7183
to `koi_certmesh::mtls`, write-authz via `ClientCn`, rake presents its enrollment cert) is BLOCKED on
that landing — plan in [`security-rake-mtls-plan.md`](security-rake-mtls-plan.md).

**UPDATE 2026-06-15 — koi SHIPPED it; integration unblocked but has a design decision.** koi promoted the
adapter into `koi_certmesh::mtls::{build_server_config, serve, extract_cn}` (dev@2ca7177; also fixed a
latent rustls-provider panic via `builder_with_provider`); `ClientCn` public. zen `cargo check
--workspace` green against it; lock synced (commit `c51f0f58`). **New finding that gates the integration:**
the **browser pond dashboard** (`pond.html`, served on :7185) does pond ADMIN over the open lobby
(`pond/ceremony`, `pond` DELETE via relative fetch) and **browsers can't do mTLS** — so gating writes to
mTLS-only :7183 breaks the dashboard's admin. That's an OPERATOR design call (browser admin stays open /
dashboard goes read-only + rake-only admin / browser cert import) — captured in the plan. Also: rake's
existing `https://:7183` path uses a no-cert client → it breaks once :7183 is mTLS-required, so the moss
flip + the rake cert must land together. The careful multi-consumer integration → a focused session.

---

## UPDATE 2026-06-22 — koi 0.5.0 leverage analysis (supersedes the mTLS-library framing above)

Ran a 10-axis cross-repo analyze→verify workflow (koi 0.5.0). Full plan:
[`koi-0.5-leverage-refactor-plan.md`](docs/notes/koi-0.5-leverage-refactor-plan.md).

**Headline:** koi's service surface exploded, but most of it is the *inverse* of zen's needs
(koi observes/bridges/serves-its-own-domains; zen controls/meshes/serves-its-own-domains).
High-value leverage is narrow → **the trust plane**. 7 of 10 axes are confirmed keep-zen.

**P0 BUILD-BREAKER (must fix first):** `koi-truststore` was DELETED from koi 0.5.0 (commit d23f6bf,
ADR-019) and replaced by the published crate `os-truststore = "0.0.2"`. zen still path-deps the dead
crate (`Cargo.toml:99`, `rake/Cargo.toml:11`, `rake/src/enrollment.rs:118,124`) → zen will not build
against current `../koi`. Fix = depend on published `os-truststore` directly; repoint
`enrollment.rs` to `os_truststore::Cert::from_pem` + `os_truststore::Install`; track installed-root
state zen-side for `is_ca_installed` (koi uses its own `state/trust.json`).

**The earlier mTLS-library finding is now resolved + reframed.** koi shipped ADR-020: the
posture/`Envelope`/`Assurance` facade (`koi_certmesh::envelope::{build_envelope,verify_envelope}`,
`CertmeshHandle::{sign,verify,seal,open}`), `serve_adaptive`/`participate` (same-port HTTP↔mTLS
live-flip), and `testkit::{open,secured}`. So the dual-mode "wishlist" is **largely already built in
koi**. zen's real gap is NOT consuming it: the UDP chirp sign/verify (`run.rs:1633-1700`) is
hand-rolled and **both spoofable** (verifies against self-asserted `sender_cert`, no CA chain,
`run.rs:1694-1696`) **and replayable** (no freshness) **and silently downgrades** (`run.rs:1681`
"accept unsigned"). Phase 1 = delegate chirp crypto to koi's `Envelope` (closes all three).
`koi-mtls-library-prompt.md` and `security-rake-mtls-plan.md` are **superseded** by this plan.

**Keep-zen tripwires (do not re-litigate):** container control plane (koi-runtime is observe-only),
L7 ollama router (koi-proxy is L4), offering/host health (koi-health is peer-liveness), DNS
(DNS-0002), UDP-7184 mesh (koi-udp rejects multicast; STACK-0001 D4), HTTP client layer, S3/content
HMAC (koi-crypto has no HMAC/SigV4). Secrets are already adopt-full (`secrets.rs` = `pub use
koi_crypto::vault`).
