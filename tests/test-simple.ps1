...
try {
    ...
    if ($response.suggestions -and $response.suggestions.Count -gt 0) {
        Write-Host "  ! Suggestions NOT suppressed: $($response.suggestions.Count)" -ForegroundColor Yellow
    } else {
        Write-Host "  OK Suggestions suppressed" -ForegroundColor Green
    }
} catch {
    Write-Host "X Error: $_" -ForegroundColor Red
}
...
try {
    ...
    if ($output -match "GARDEN OVERVIEW") {
        Write-Host "OK Zen observe command works" -ForegroundColor Green
    } else {
