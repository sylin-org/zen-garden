...
set IMAGE=sylinorg/zen-garden-mongodb-orchestrator
set TAG=latest
set CONTAINER=zen-garden-mongodb-orchestrator
...
echo.
echo  Zen Garden MongoDB Orchestrator
echo  ────────────────────────────────
...
docker run -d ^
    ...
    -v zen-garden-mongodb-data:/data ^
    --restart unless-stopped ^
    %IMAGE%:%TAG%

echo.
echo  Container started: %CONTAINER%
echo  Dashboard: http://localhost:7191
echo.
