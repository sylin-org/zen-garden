...
**Discussion:**
- Pros: Consistency (Moss/Rake run on Windows)
- Cons: Different audio APIs (WASAPI vs ALSA), additional testing

**Decision:** Phase 4 (post-Linux release). Focus on Linux first (Wyse 5070 testbed).
...
**Discussion:**
- Pros: Consistent deployment, no systemd
- Cons: Audio device passthrough tricky, adds latency

**Decision:** No. Cricket runs bare metal for direct audio access. Moss manages containers; Cricket is a companion service.

### Issue 3: Sample Copyright

**Question:** Can we redistribute CC0 samples in binary?

**Discussion:**
- CC0 allows unrestricted use/distribution
- Include `SAMPLES.md` with attributions (courtesy, not required)
- Verify each sample's license on Freesound.org

**Decision:** Yes, embed samples in binary. Document sources in `SAMPLES.md`.
...
1. **Create Repository:** `zen-garden/garden-cricket`
2. **Set Up Cargo Project:** `cargo init --bin garden-cricket`
3. **Implement Phase 1:** Minimal viable Cricket (2 weeks)
4. **Deploy to Wyse 5070:** Test with real Moss
5. **Iterate Based on Feedback:** Adjust volume, sample selection
6. **Implement Phase 2:** Full layer system (2 weeks)
7. **Beta Testing:** 2-3 additional stones (1 week)
8. **Implement Phase 3:** Polish + multi-stone (2 weeks)
9. **Release v0.1.0:** Announce to Zen Garden users

**Timeline:** 7-8 weeks from start to production release
...
**Document Status:** Planning → Implementation  
