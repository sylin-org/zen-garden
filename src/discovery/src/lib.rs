//! Stone discovery for clients of the Zen Garden.
//!
//! mDNS browse, UDP multicast/broadcast, parallel auto-discovery, and
//! Lantern + certmesh CA resolution. Consumed by Rake (CLI), Pavilion
//! (Windows tray client), and Lantern (passive listener bridge).
//!
//! See [DISC-0001](../../../docs/decisions/DISC-0001-discovery-as-first-class-crate.md)
//! for the architectural decision behind this crate.
//!
//! # Migration note
//!
//! This file was lifted from `src/rake/src/discovery.rs` per DISC-0001.
//! Internal organisation (split by transport into `mdns.rs`, `udp.rs`,
//! etc.) is a follow-up refactor — for now everything lives in this
//! single module to keep the move surgical. The TTL cache and
//! `DiscoveryProvider` trait moved out of `garden-common` into this
//! crate as part of DISC-0001 cleanup; see [`cache`] and [`provider`].

pub mod cache;
pub mod provider;

pub use cache::{Discovery, KnownStone, STONE};
pub use provider::{DefaultDiscoveryProvider, DiscoveryError, DiscoveryProvider, DiscoveryResult};
