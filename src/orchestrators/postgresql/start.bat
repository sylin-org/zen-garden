...
set IMAGE=sylinorg/zen-garden-postgresql-orchestrator
set TAG=latest
set CONTAINER=zen-garden-postgresql-orchestrator
...
echo.
echo  Zen Garden PostgreSQL Orchestrator
echo  ────────────────────────────────────
...
docker run -d ^
    ...
    -v zen-garden-postgresql-data:/data ^
    --restart unless-stopped ^
    %IMAGE%:%TAG%

echo.
echo  Container started: %CONTAINER%
echo  Dashboard: http://localhost:7192
echo.
