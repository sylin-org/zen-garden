...
use garden_kernel::presence::PeerView;
use serde::Serialize;
use std::sync::Arc;
use uuid::Uuid;
...
pub struct AppState {
    pub presence: Arc<garden_kernel::presence::Presence>,
...
impl From<&PeerView> for ObserveStone {
    fn from(peer: &PeerView) -> Self {
...
const MANIFEST: [ManifestRoute; 3] = [
    ManifestRoute {
        method: "GET",
        path: "/health",
        summary: "Liveness of this stone and its wire protocol marker.",
    },
    ManifestRoute {
        method: "GET",
        path: "/api/v1/observe",
        summary: "The garden as this stone sees it: every peer's last chirp.",
    },
    ManifestRoute {
        method: "GET",
...
async fn observe(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
    let stones: Vec<ObserveStone> =
        state.presence.snapshot().iter().map(ObserveStone::from).collect();
...
pub fn router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route("/api/v1/observe", get(observe))
...
mod tests {
    ...
    async fn every_manifest_route_answers() {
        ...
        let state = Arc::new(AppState {
            presence: Arc::new(garden_kernel::presence::Presence::new()),
    ...
    fn observe_stone_flattens_chirp_body() {
        let view = ObserveStone::from(&sample_peer());
