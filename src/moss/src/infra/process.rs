...
pub async fn kill_existing_moss_processes_graceful() -> anyhow::Result<()> {
    let shutdown_url = format!("http://127.0.0.1:{}/admin/shutdown", garden_common::ports::MOSS_HTTP);
    process::kill_process_graceful(garden_common::names::MOSS_BINARY, &shutdown_url).await
}
...
pub fn check_moss_processes_exist() -> bool {
    process::check_process_exists(garden_common::names::MOSS_BINARY)
}
...
pub fn kill_existing_moss_processes() -> anyhow::Result<()> {
    process::kill_process(garden_common::names::MOSS_BINARY)
}
