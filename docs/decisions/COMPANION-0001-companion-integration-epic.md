...
| 2026-04-13 | **Book V closed.** Garden Aggregate — `Garden` (Arc-shared, private `GardenState`, subscribes to `Pulse`), `GardenSnapshot` synthetic payload (`core.garden.snapshot` kind), `GardenSubscription { snapshot: Event, receiver }` return from `subscribe()` (refinement over pattern spec's wrapped-receiver idea — plain broadcast can't target a single new subscriber). Projection updates state for 7 core event kinds (presence.snapshot / stone.health.changed / stone.load.updated / service.started / service.stopped / storage.connected / storage.removed). Added `Default` impls to Book IV's `Health` / `Pond` / `Load` (Dormant / Solo / ZERO). Typed accessors: `stone_name`, `health`, `load`, `offerings`, `seed_bank`, `pond`, `is_ready`, `snapshot`. 14 unit tests (default state, 8 projection cases, 3 subscribe/event cases, 2 async projection-task cases). 99 garden tests total, all green. Zero new workspace deps. | [COMPANION-0006](COMPANION-0006-garden-aggregate.md) |
| 2026-04-13 | **Book IV closed.** Domain Types — `garden-common::domain` module with `Health` (5-valued enum, serde-compatible with existing `VITALITY_*` wire strings), `Load` (typed `Percent` + all resource fields), `SeedBank` (with derived `free_gb`/`fill_percent`), `Pond` (Solo/Member/Cornerstone). Re-exports `Stone`, `OfferingFqn`, `StoneStatus` from their canonical locations so one import path covers all domain values. SDK core payloads gain extension traits (`StoneHealthChangedExt::health_domain`, `StoneLoadUpdatedExt::load_domain`, `PresenceSnapshotExt::stone_health/stone_load/seed_bank/pond`). Scope refinement: moss internal migration deferred post-epic (strings continue to work; Book IV is *vocabulary*, not *behavior*). Zero new workspace deps. 31 domain tests + 5 SDK accessor tests, 85 garden tests overall. | [COMPANION-0005](COMPANION-0005-domain-types.md) |
| 2026-04-13 | **Book III closed.** Transport — `Transport` trait + two impls (`SseTransport`, `CommandTransport`) + core payload types. Nine typed payloads cover moss's current presence-event surface (`core.presence.snapshot`, `core.stone.{health.changed,load.updated,tended}`, `core.service.{started,stopped}`, `core.storage.{connected,detected,removed}`). `SseTransport` acts as the anti-corruption layer: translates moss's two-level wire kinds into our three-level `core.*` namespace via `wire_to_core_kind`. `CommandTransport` translates `POST /command` into `CommandInvocation` events, correlates `CommandResult` events back via an in-memory correlation map + `mpsc` channel, aggregates 0/1/N results into a single `CommandResponse`. Three axum endpoints (`/command`, `/shutdown`, `/health`). Three scaffold entries registered (`companion-old-sse-client`, `companion-old-command-handler`, `companion-old-companion-runtime`) — all with removal trigger Book VIII Ch6. One new direct dep (`tokio-util = "0.7"`, mirroring moss). 58 new unit + integration tests; 80 garden tests total, all green. | [COMPANION-0004](COMPANION-0004-transport.md) |
