...
//!
//! Pure computation — no I/O, no locks. Determines the ideal model->stone
//! assignment based on recent demand distribution and VRAM constraints.
//!
//! Generalized from ollama-orchestrator domain/placement.rs — operates on
//! `ServiceInstance` and `ModelInfo` instead of `OllamaInstance`.
...
/// 3. Models with `demand_share x num_stones >= 1.5` get extra replicas.
...
pub fn compute_placement(
    ...
    // Compute ideal replica count per model. Models whose VRAM has never
    // been measured are skipped — cannot safely bin-pack unknown sizes.
    ...
    let mut stone_remaining: Vec<(String, u64)> = healthy
        .iter()
        .map(|i| (i.endpoint.clone(), i.vram.budget_bytes))
        .collect();
...
mod tests {
    ...
    fn inst(name: &str, ep: &str, vram_gb: u64) -> ServiceInstance {
        ServiceInstance {
            stone: Stone {
                id: String::new(),
                name: name.to_string(),
