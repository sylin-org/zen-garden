...
impl TuneManager {
    ...
    pub fn new(fs_tunes_dir: Option<&str>) -> Result<Self> {
        ...
        tracing::info!(
            total = tunes.len(),
            embedded = tunes.values().filter(|t| t.source == TuneSource::Embedded).count(),
            filesystem = tunes.values().filter(|t| matches!(t.source, TuneSource::Filesystem(_))).count(),
    ...
    fn scan_filesystem_tunes(tunes_dir: &Path) -> Result<HashMap<String, LoadedTune>> {
        ...
        let entries = std::fs::read_dir(tunes_dir)
            .context("Failed to read tunes directory")?;
    ...
    fn load_manifest_file(path: &Path) -> Result<TuneManifest> {
        ...
        let manifest: TuneManifest = serde_yaml::from_str(&content)
    ...
    pub fn select(&self, name: &str) -> Result<()> {
        if !self.tunes.contains_key(name) {
            anyhow::bail!("Tune '{}' not found. Available: {:?}", 
                name, 
    ...
    pub fn resolve_resource_bytes(&self, tune_name: &str, resource: &str) -> Option<Vec<u8>> {
        ...
        match &tune.source {
            TuneSource::Embedded => {
                ...
                EmbeddedTunes::get(&embedded_path)
                    .map(|f| f.data.to_vec())
            }
    ...
    pub fn resolve_resource_bytes_with_fallback(&self, tune_name: &str, resource: &str) -> Option<Vec<u8>> {
