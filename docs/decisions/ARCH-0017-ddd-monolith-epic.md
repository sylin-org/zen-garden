...
| Date | Change | ADR |
|------|--------|-----|
| 2026-04-12 | **Book XVIII opened.** Offerings Strangler Removal — plan confirmed. 81 `.read().await` sites across 31 files, all map to existing typed queries. No new query methods needed. `offering_lifecycle.rs` query functions and `AppState` delegates also removed. | [ARCH-0036](ARCH-0036-offerings-strangler-removal.md) |
...
| 2026-04-12 | **Book XVI closed.** EventBus / Pulse — plan change: unification → dissolution. EventBus (3 listeners: chirp, pulse bridge, timer) and Pulse (SSE firehose merging domain + transport) serve different event populations from per-aggregate `changes()` channels. No domain state, no invariants. Proposed `PulseProjectionTask` would duplicate existing call-site emit pattern. 764 tests. | [ARCH-0034](ARCH-0034-eventbus-pulse-dissolution.md) |
| 2026-04-12 | **Book XV closed.** Logging — plan change: aggregate → dissolution. `AppState::log` is a single `broadcast::Sender<String>` with 1 consumer (SSE stream). `LogBroadcastLayer` is correctly in `infra/`. File sink managed by `tracing-appender`. No domain state, no invariants, no events beyond raw tracing output. 764 tests. | [ARCH-0033](ARCH-0033-logging-dissolution.md) |
| 2026-04-12 | **Book XIV closed.** Persistence Consolidation — plan change: shared helpers → dissolution. `garden_common::persistence` already provides `atomic_write_file()`, `JsonStorage<T>`, `PersistenceProvider<T>` — the consolidation target already exists. Store port adapters are thin delegates (10-20 lines). `DirectoryCache<K, V>` has one consumer. Inline atomic write duplicates are mechanical cleanup, not domain architecture. 764 tests. | [ARCH-0032](ARCH-0032-persistence-consolidation-dissolution.md) |
| 2026-04-12 | **Book XIII closed.** Configuration — plan change: aggregate → dissolution. Config is loaded once at boot and frozen; no mutable state, no invariants, no events. `MossConfig` + `DaemonConfig` are pure value objects. 6 dead timeout fields/accessors deleted. `EnvConfig` stays in `garden-common` (cross-crate). 764 tests. | [ARCH-0031](ARCH-0031-configuration-dissolution.md) |
| 2026-04-12 | **Book XII closed.** ContainerRuntime Port — plan change: full trait abstraction → rename + seal + delete. `docker::Client` already IS the anti-corruption layer; dead `ServiceRuntime` trait + `infra/container.rs` deleted, Bollard leak sealed via `ContainerEvent`, `docker::Client` → `docker::ContainerRuntime`, `Platform.docker` → `Platform.container`, 764 tests. | [ARCH-0030](ARCH-0030-container-runtime-port.md) |
