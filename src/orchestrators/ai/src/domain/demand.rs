...
//!
//! Three decay windows per counter:
...
//!
//! Generalized from ollama-orchestrator domain/demand.rs — `RequestCapability`
//! replaced with unified `Capability` enum.
...
pub const REACTIVE_HALF_LIFE_SECS: f64 = 15.0 * 60.0; // 15 minutes

/// Half-life for the tactical window (placement, workload separation).
pub const TACTICAL_HALF_LIFE_SECS: f64 = 6.0 * 3600.0; // 6 hours

/// Half-life for the strategic window (replication, eviction).
pub const STRATEGIC_HALF_LIFE_SECS: f64 = 3.0 * 86400.0; // 3 days
...
impl DemandLedger {
    ...
    /// Per-capability demand distribution (tactical window).
    ///
...
mod tests {
    ...
    fn observed_fitness_tracking() {
        ...
        for _ in 0..5 {
            ledger.record_request(now, Capability::Chat, "llama3:8b", "stone-a", 50, 1_000_000_000);
        }
