...
| ~~D17~~ | ~~Capability model files written by in-container uid 0 — non-root uproot cannot delete them~~ | — | **Closed 2026-08-29:** uproot now removes the offering's whole directory (previously left behind forever), and when uid-0 workload files resist the moss's uid, the WORLD purges via a one-shot container from the offering's OWN image (no new pulls); a world that cannot purge degrades with an honest warning, never a failed uproot || D18 | Release artifacts carry sha256 checksums only — no cryptographic signature | M6 (Koi integration: key ceremony + verify-chain) | The M1 trust anchor is TLS + the checksum manifest; B2's named trust anchor lands with M2 signing |
