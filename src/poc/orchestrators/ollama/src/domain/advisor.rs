...
use super::demand::RequestCapability;
...
pub struct DemandContext {
    ...
    pub capability_distribution: HashMap<RequestCapability, f64>,
    ...
    pub capability_rates: HashMap<RequestCapability, f64>,
}
...
/// Minimum VRAM headroom to preserve for Ollama runtime overhead.
...
pub fn advise_topology(
    ...
    let uniform = 1.0 / RequestCapability::ALL.len() as f64;
    let cap_demand = |cap: RequestCapability| -> f64 {
        demand
            .and_then(|d| d.capability_distribution.get(&cap))
            .copied()
            .unwrap_or(uniform)
    };
    let embedding_demand = cap_demand(RequestCapability::Embedding);
    let thinking_demand = cap_demand(RequestCapability::Thinking);
...
///
/// Now includes fitness data when available (ORCH-0009).
        ...
        .map(|i| {
            ...
            let observed_tps = demand_ledger.and_then(|dl| {
                ...
                let tps_values: Vec<f64> = i
                    .models_available
                    .iter()
                    .filter_map(|m| dl.observed_tps(m, &i.stone_name))
                    .collect();
...
mod tests {
    ...
    fn demand_context_affects_parallelism() {
        ...
        let demand = DemandContext {
            capability_distribution: [(RequestCapability::Embedding, 0.8), (RequestCapability::Chat, 0.2)]
                .into_iter()
                .collect(),
            confidence: 1.0,
            ..Default::default()
