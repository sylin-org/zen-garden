...
//!
//! Generalized from ollama-orchestrator domain/metrics.rs — `MetricEvent`
//! now carries unified `Capability` instead of `RequestCapability`.
...
impl MetricsEngine {
    ...
    /// Compute roundtrip tok/s per stone (total_duration).
    pub fn roundtrip_tokens_per_sec_by_stone(&self, window_secs: u64) -> HashMap<String, f64> {
        let cutoff = Instant::now() - std::time::Duration::from_secs(window_secs);
        let mut per_stone: HashMap<&str, (u64, u64)> = HashMap::new();
        for (t, stone, tok, _, total_dur) in &self.stone_throughput {
            if *t > cutoff {
                let e = per_stone.entry(stone.as_str()).or_default();
                e.0 += tok;
                e.1 += total_dur;
    ...
    /// All-time generation tok/s for a single stone.
    ...
    /// All-time roundtrip tok/s for a single stone.
    ...
    /// Per-model demand shares over a time window.
    ...
    pub fn process_event(&mut self, event: MetricEvent) {
        match event {
            MetricEvent::Request {
                ...
                self.record_request(
                    &stone, &model, tokens_in, tokens_out, duration_ns, eval_duration_ns,
                );
            }
            MetricEvent::Error {
                stone,
