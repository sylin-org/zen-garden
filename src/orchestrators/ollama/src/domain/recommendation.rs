...
/// Map a user-facing capability to the Ollama model capability tag used for filtering.
...
mod tests {
    ...
    fn vision_and_ocr_produce_different_rankings() {
        // Vision: no quality/name-affinity scoring (caps are 0 for vision by default?
        // Actually vision has no special caps — let's check it's different from OCR.
        let mut models = HashMap::new();
        models.insert(
            "llava:latest".to_string(),
            make_model_with_params(
