...
/// Request capability / intent, inferred from the proxy request.
///
/// Path-level inference is coarse (embed vs everything else). For richer
/// classification the proxy inspects the request body: presence of a
/// `tools` array → Tools, model tagged with "thinking" → Thinking, image
/// content → Vision.
...
impl RequestCapability {
    /// Coarse inference from the Ollama API path alone.
    ///
    /// For richer classification use [`Self::from_request`] which inspects
    /// the request body and model capabilities.
    pub fn from_path(path: &str) -> Self {
        match path {
            "/api/embed" | "/api/embeddings" => Self::Embedding,
            _ => Self::Chat,
        }
    }

    /// Richer inference from path + parsed request body + model capabilities.
    ///
    /// Priority (first match wins):
    /// 1. Embedding endpoints → Embedding
    /// 2. `tools` array present → Tools
    /// 3. Image content in messages → Vision
    /// 4. Model has "thinking" capability → Thinking
    /// 5. Fallback → Chat
    pub fn from_request(
        path: &str,
        ...
        // 1. Embedding endpoints are unambiguous
        if matches!(path, "/api/embed" | "/api/embeddings") {
            return Self::Embedding;
        }

        // 2. Tools array present in the request body
        if body.get("tools").and_then(|t| t.as_array()).is_some_and(|a| !a.is_empty()) {
            return Self::Tools;
        }
        ...
        let has_images = body
            ...
            // /api/generate uses top-level "images"
            || body
                .get("images")
                .and_then(|i| i.as_array())
                .is_some_and(|a| !a.is_empty());
        ...
        // 4. Model with thinking capability
        if model_capabilities.iter().any(|c| c == "thinking") {
    ...
    /// Map a moniker capability string to a `RequestCapability` for demand tracking.
    pub fn from_moniker(cap: &str) -> Self {
        match cap {
            "embedding" => Self::Embedding,
            "vision" | "ocr" => Self::Vision,
            "tools" => Self::Tools,
            "thinking" => Self::Thinking,
            _ => Self::Chat, // quick, chat, completion, synthesis
        }
    }
...
impl std::fmt::Display for RequestCapability {
...
pub struct DemandLedger {
    ...
    pub by_capability: HashMap<RequestCapability, DecayCounter>,
...
impl DemandLedger {
    ...
    pub fn capability_distribution(&self, now: Instant) -> HashMap<RequestCapability, f64> {
        let confidence = self.confidence();
        let n_capabilities = RequestCapability::ALL.len() as f64;
        let uniform_weight = 1.0 / n_capabilities;

        if confidence < f64::EPSILON {
            // No data: pure uniform
            return RequestCapability::ALL
                .iter()
        ...
        let mut observed: HashMap<RequestCapability, f64> = HashMap::new();
    ...
    pub fn capability_rates(&self, now: Instant) -> HashMap<RequestCapability, f64> {
        self.by_capability
            .iter()
            .map(|(&cap, counter)| {
...
mod tests {
    ...
    fn demand_ledger_confidence_ramp() {
        ...
        let now = Instant::now();
        for _ in 0..25 {
            ledger.record_request(now, RequestCapability::Chat, "llama3:8b", "stone-a", 100, 1_000_000_000);
        }
    ...
    fn capability_distribution_uniform_at_zero() {
        ...
        let n = RequestCapability::ALL.len() as f64;
        for &cap in RequestCapability::ALL {
    ...
    fn capability_distribution_converges() {
        ...
        for _ in 0..100 {
            ledger.record_request(now, RequestCapability::Embedding, "nomic-embed", "stone-a", 0, 0);
        }

        let dist = ledger.capability_distribution(now);
        let embedding_share = dist.get(&RequestCapability::Embedding).copied().unwrap_or(0.0);
        // Should be dominant (close to 1.0 since confidence is 1.0)
        assert!(
            embedding_share > 0.8,
            "expected embedding > 0.8, got {embedding_share}"
        );
    }
